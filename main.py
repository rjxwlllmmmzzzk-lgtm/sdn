...
SUFFIXES = [
    ...
    "يا ابن العاهره", "يا ابن الدعاره", "يا خول", "يا جرار", "يا منيوك"
]
...
LONG_TAKLEESH_WORDS = [
    ...
    "اقتحم نسلك يا ابن الزنا واخلي عيالك عبيد",
    "اخدر امك يا ابن الكلب واخليها ترقص عريانه",
    "انيج امك الكحبه يا ابن العرص 100 مره",
    "ربك اسمطه يا ابن الفاعله واخليك تصيح",
    "اعبد زبي يا خول واخليك تقبل الارض",
    "المخنث يا ابن الديوث حتي خواتك ما يبونك",
    "اطشر صريمك يا ابن الحمار واخليك تنام بالمستشفى",
    "افلشك تفليش يا ابن البقره واخليك تشحت",
]
...
async def send_tasteer_messages(user_id, target, delay, chat_id):
    ...
    if not client:
        await bot.send_message(chat_id, "❌ خطأ في الجلسة")
        return
    tasteer_list = STRONGS + SUFFIXES + [
        "كس امك يا ابن القحبه", "كسمك يا شرموطه", "كس اختك يا ديوث",
        "انيك امك يا خنيث", "ديوث يا ابن الديوث", "خنيث يا ابن الخنيث"
    ]
    for i in range(3):
        if active_spams[user_id]["stop"]:
            await bot.send_message(chat_id, "🛑 تم الإيقاف")
...
# =============== أجزاء الكليشات للتوليد العشوائي ===============
PREFIXES = [
    "لحلكك", "اشيلك", "عبالك", "انيجمك", "مصمص", "اهف", "اربطك", "اطحن", "اكهرب", "احط", 
    "اقتحم", "اخدر", "انيج", "ربك", "اعبد", "ادحس", "افلش", "اذب", "اكعد", "ازورك", 
    "اصعق", "اطشر", "اعجن", "اشكه", "اشنقك", "ابعبص", "اتفل", "اتسودن", "اتنايج", "اخرمش"
]
...
MIDDLES = [
    ...
    "كس اختك", "كس امك", "عيري", "جبتي", "ربعك", "خواتك", "عيالك", "نياكة"
]
...
STRONGS = [
    ...
    "احرق بيتكم", "اخرب عليك", "افضحك بين الناس", "احرجك قدام خواتك", "اخليك تندم", 
    "اخلي عيالك يبكون", "اخلي زوجتك تطلق منك", "اخلي اهلك كلهم يعرفون قرفك"
]

# =============== توليد كليشة عشوائية طويلة ===============
def generate_long_takleesh():
    """توليد كليشة طويلة مكونة من 5-8 كلمات"""
    parts = []
    
    # الجزء الأول: فعل + مفعول
    parts.append(random.choice(PREFIXES))
    parts.append(random.choice(MIDDLES))
    
    # الجزء الثاني: إضافة قوة عشوائية (50% فرصة)
    if random.random() > 0.5:
        parts.append(random.choice(STRONGS))
    ...
    # إضافة كلمات إضافية (30% فرصة)
    if random.random() > 0.7:
        parts.append("يا")
        parts.append(random.choice(SUFFIXES).replace("يا ", ""))
    
    return " ".join(parts)

# =============== كليشات جاهزة طويلة ===============
LONG_TAKLEESH_WORDS = [
    "لحلكك الهالبك طيزمك يا ابن الكلب يا شرموطه انيك كس امك",
    "اشيلك بعيري يا ابن الشرموطه كس اختك يا ديوث انيك عرضك",
    "عبالك اعوفك؟ اني المفروض انيك كس امك واخليك تندم يا ابن القحبه",
    "انيجمك علصدرك يا ابن الزانيه واطحن مخك وافطر كبدك يا كلب",
    "مصمص عيورتي يا ابن العاهره واخليك تلحس جزمتي يا خنيث",
    "اهف اختك بطرف عيري يا ديوث واخليها تصير شرموطه في الشارع",
    "اربطك بقياطين قندرتي يا ابن الكحبه واسحبك بالشوارع يا جبان",
    "اشيل ربك واركعه بلكاع يا خنيث واخليك تركض ورا عيري يا تيس",
    "اطحن ضلوعك يا ابن المتناكه واخلطها بالاسمنت وابني فيها بيت",
    "اكهرب طيزك يا ابن الشرموطه واخليك تقفز زي القرد يا حمار",
    "احط قضيبي بكس امك يا ابن الدعاره قدام اخواتك واهلك كلهم",
    "اقتحم نسلك يا ابن الزنا واخلي عيالك كلهم عبيد عندي",
    "اخدر امك يا ابن الكلب واخليها ترقص عريانه في الشارع",
    "انيج امك الكحبه يا ابن العرص 100 مره قدام الحلال والحرام",
    "ربك اسمطه يا ابن الفاعله واخليك تصيح زي البيبي",
    "اعبد زبي يا خول واخليك تقبل الارض اللي يمشي عليها",
    "المخنث يا ابن الديوث حتي خواتك ما يبونك يا جبان",
    "اطشر صريمك يا ابن الحمار واخليك تنام في المستشفى شهر",
    "افلشك تفليش يا ابن البقره واخليك تشحت على بوابات المساجد",
    "اذب تيزاب بكسمك يا ابن الخنزير واخلي وشك يذوب زي الشمع",
    "انيج رب ربك يا ابن الكس واخليك تصيح يا أستاذ",
    "افترس طيزك يا ابن الوسخه واخليك تركض ورا عيري يا تيس",
    "ادحس البنكه بصرمك يا ابن المتناك واخلي البنك كله يخاف منك",
    "احط الويسكي بكس اختك يا ديوث واخليها تشرب وتدور عير",
    "اشكشك ديوسك يا ابن القحبه واخلي كسك يتحول لفضيحة",
    "اربطك بقياطين قندرتي يا ابن الزانيه واسحبك من طيزك",
    "ادحس الكعبه بطيزك يا ابن الشرموطه واخلي الحجاج يندبون حظهم",
    "ازورك وجهك بعيري يا ابن الكلب واخلي صورتك تصير ميمز",
    "اصابعي تفترسك يا ابن العرص واخليك تصيح يا خنزير",
    "اختك كحبتي يا ديوث واخليها تنام مع كل الكلاب في الحي",
    "اله اشكه لحلكك يا ابن الشرموطه واخليك تلحس التراب",
    "انزل لعنه عيري بكسمك يا ابن الخنزير واخليك تلعن اليوم اللي جيت فيه",
    "شو دتعال يا جبان حتي امك خذلانة فيك يا ديوث",
    "ابن المتبربكه يا ابن القحبه أنت وكل عيالك كلاب شوارع",
    "دشوف شراح اسوي فيك يا ابن الكلب راح اخليك تندم انك انولدت",
    "اتفل بصرمك يا ابن الخول واخلي وجهك كله بزاز",
    "احط عيورتي بطيزمك يا ابن الزاني واخليك تجلس طول عمرك",
    "ادحس رجلي بكس اختك يا ديوث واخليها تصير معاقه جنسيا",
    "ولك تعال مصعيري يا ابن الشرموطه واخليك تلحس عيري",
    "انكز على صريمك يا ابن العرص واخلي نياكتك تصير مشهوره",
    "اشنقك بلباس امك يا خنيث واخليك تموت على طريقة المخانيث",
    "احط امك بطيزك يا ابن المتناكه واخليك تنيك حالك",
    "ابن الهايته يا ابن الدعاره أنت وعيالك كلاب حي",
    "امك احطها بعيري يا ابن الزنا واخليها تموت من العار",
    "ابن الربل يا ابن الكلب تربيت في الزباله وكلبك احسن منك",
    "ابن مصاصه عيورتي يا خول من يومك وانت تمص زبي",
    "اتناطح وي عنابه امك يا ابن القحبه واخلي الجيران كلهم يضحكون عليك",
    "اصعد على ضهرك يا ابن الشرموطه واخليك حمار ينقل الحطب",
    "نياج اختك اني يا ديوث واخليها تحمل مني وتجيب عيال زيك",
    "ادحس البنكه بصرمك يا ابن المتناك واخلي البنك يسكر بوجهك",
    "ابعبص صريمك يا ابن العرص واخليك تنيك نفسك بالمرايه",
    "افلش كسمك تفليش يا ابن الكلب واخليك تشحت من عيالك",
    "احط الويسكي بكس اختك يا خنيث واخليها تشرب وتسكر وتدور عير",
    "اتسودن عليك يا ابن الشرموطه واخليك تصير عبد عند عبيدي",
    "ابن كحباتي يا ابن القحبه كل خواتك كحبات وانت ديوث كبير",
    "اشلع شفايف امك يا ابن الزانيه واخليها تصير فضيحه في الحاره",
    "اجنكل طيز اختك يا ديوث واخليها تصير مليانه تقرحات",
    "ابن بلاعت العير يا ابن المتناكه طول عمرك وانت تبلع زبي",
    "اجك امك بعيري يا ابن الكلب واخليك تصيح يا جبان",
    "امصمص ديوس اختك يا خول واخليك ديوث على راحتك",
    "ابن المراهقه يا ابن القحبه أنت حمل مراهقه وطلع كلب",
    "اهف راس امك بل طاوه يا ابن الشرموطه واخليها تموت من الضربه",
    "اشكشك ديوسك يا ابن العرص واخلي نياكتك تصير حلقه تيك توك",
    "اربطك بقياطين قندرتي يا ديوث واسحبك من طيزك في الشارع",
    "اخلي تمص جبتي يا خنيث واخليك تلحس عيري طول عمرك",
    "اسوي كسيسمك طشار يا ابن القحبه واخلي كسك يتقطع",
    "اجب بلباس اختك يا ديوث واخليك ترقص زي الحرمه",
    "اصمل ولك يا ابن الشرموطه واخلي عينك تصير زرقا"
]
...
async def send_takleesh_messages(user_id, target, count, chat_id):
    ...
    for i in range(count):
        ...
        # اختيار عشوائي بين الكليشات الجاهزة والكليشات المولدة
        if random.random() < 0.6:
            word = random.choice(LONG_TAKLEESH_WORDS)
        else:
            word = generate_long_takleesh()
...
async def send_tasteer_messages(user_id, target, delay, chat_id):
    ...
    tasteer_list = STRONGS + SUFFIXES + [
        "كس امك يا ابن القحبه", "كسمك يا شرموطه", "كس اختك يا ديوث", "انيك امك يا خنيث",
        "كسم امك يا ابن العاهره", "ديوث يا ابن الديوث", "خنيث يا ابن الخنيث", "شرموطه يا ابن الشرموطه",
        "قحبه يا ابن القحبه", "خول يا ابن الخول", "قواد يا ابن القواد", "منيوك يا ابن المنيوك",
        "انيك كس امك يا كلب", "اطحن مخك يا حمار", "افطر كبدك يا خنزير", "اشرب دمك يا تيس",
        "احرق بيتكم يا جبان", "اخرب عليك يا ديوث", "افضحك بين الناس يا شرموطه"
    ]
...
async def successful_payment(message):
    ...
    await bot.reply_to(message, f"✅ تم تفعيل اشتراكك لمدة {name}")
...
async def gift_subscription(message):
    ...
    try:
        ...
        if hours is None:
            await bot.reply_to(message, "❌ صيغة غير صالحة")
            return
        add_subscription(target_id, hours)
        if hours < 1:
        ...
        await bot.reply_to(message, f"✅ تم تفعيل اشتراك {target_id} لمدة {duration_str}")
        try:
            await bot.send_message(target_id, f"🎁 تم تفعيل اشتراك لك لمدة {duration_str}")
        except:
            pass
    except:
        await bot.reply_to(message, "❌ خطأ")
